#!/usr/bin/env python3
"""rebuild -- the single self-contained script that reproduces every manuscript
figure, data table, and the PDFs from original sources, in one command.

This file is the ONE "big fat" rebuild: every former ``analysis/*.py`` figure and
helper module has been inlined here (flat namespace; module-private names are
prefixed ``_<module>__name``). There are no more per-figure modules to run by
hand -- each stage is a function in this file and runs in-process. The only
remaining external modules are ``analysis.plexDIA.*`` (run as subprocesses) and
the LaTeX/PDF build (``paper/Makefile``).
...
the filter rule documented in ``methods.md`` section 1.
...
The atlas (Fig S13) needs a numpy<2 environment (see methods.md / environment.yml);
run rebuild inside the ``quantmsdiann`` conda env to reproduce it.
"""
...
# Self-alias: the former ``from analysis import figure_style as fs`` (and the
# ``figure_proteobench_accuracy``/``acc`` alias) now point at THIS module, so any
# residual dynamic ``fs.<attr>`` lookups still resolve to the inlined functions.
fs = sys.modules[__name__]
figure_style = sys.modules[__name__]
...
# ===========================================================================
# Inlined from analysis/compute_cohort_consistency.py
# ===========================================================================
_compute_cohort_consistency___CONTAM_RE = re.compile('(?:^|;)(?:Cont_|CONTAM_|ENTRAP_|DECOY_|decoy_)', re.IGNORECASE)

def _is_target(pg: str | None) -> bool:
    return not _compute_cohort_consistency___CONTAM_RE.search(pg or '')

def compute(parquet_path, run_to_group, q=0.01, min_detection_fraction=0.0, batch_size=2000000):
    ...
    cols = ['Run', 'Protein.Group', 'Global.Q.Value', 'Proteotypic', 'Stripped.Sequence', 'Global.PG.Q.Value']
    have = set(pf.schema_arrow.names)
    cols = [c for c in cols if c in have]
    pg_runs: dict[str, set[str]] = {}
    pg_pep: dict[str, set[str]] = {}
    pg_global: set[str] = set()
    ...
    return {'group_pg': {k: sorted(v) for k, v in group_pg.items()}, 'per_run_protein_count': {r: len(pgs) for r, pgs in per_run.items()}, 'stringent_tgt': sum((1 for pg, ps in pg_pep.items() if len(ps) >= 2 and _is_target(pg))), 'stringent_unf': sum((1 for ps in pg_pep.values() if len(ps) >= 2)), 'proteins_global_tgt': sum((1 for pg in pg_global if _is_target(pg))), 'proteins_global_unf': len(pg_global), 'total_mapped_runs': total_mapped}

def _compute_cohort_consistency__main(argv=None) -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument('--parquet', required=True)
    ap.add_argument('--run-group', required=True, help='JSON: {run_basename: group_label}')
    ...
    agg = compute(args.parquet, run_to_group, q=args.q, min_detection_fraction=args.min_detection_fraction)
    json.dump(agg, open(args.out, 'w'))
    print(f"wrote {args.out}: {len(agg['group_pg'])} groups, proteins_global_tgt={agg['proteins_global_tgt']}, stringent_tgt={agg['stringent_tgt']}")
    return 0
...
_CONTAM_RE = re.compile('(?:^|;)(?:Cont_|CONTAM_|ENTRAP_|DECOY_|decoy_)', re.IGNORECASE)
_NEEDED_COLS = ['Run', 'Precursor.Id', 'Protein.Group', 'Stripped.Sequence', 'Q.Value', 'Global.Q.Value', 'Global.PG.Q.Value', 'PG.Q.Value', 'Decoy']
...
def count_report(df: pd.DataFrame, precursor_q: float=DEFAULT_PRECURSOR_Q) -> dict[str, int]:
    ...
    def counts(mask: pd.Series) -> tuple[int, int, int]:
        ...
        proteins = d.loc[d['Global.PG.Q.Value'] <= Q_THRESHOLD, 'Protein.Group'].nunique()
    ...
    if 'PG.Q.Value' in df.columns:
        ...
        pg = df[(df['PG.Q.Value'] <= Q_THRESHOLD) & is_target][['Run', 'Protein.Group']].drop_duplicates()
    ...
    return {'prec_min1_tgt': t1, 'prec_min1_unf': u1, 'prec_min3_tgt': t3, 'prec_min3_unf': u3, 'proteins_tgt': tp, 'proteins_unf': up, 'proteins_2pep_tgt': prot_2pep_tgt, 'peptides_tgt': pep_tgt, 'prot_avg_tgt': prot_avg, 'prot_complete_tgt': prot_complete}

def _count_report_ids__main(argv: list[str] | None=None) -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ...
    for module in DATASET_MODULES:
        for version in _count_report_ids__VERSIONS:
            ...
            rows.append(c)
            print(f"{module} {version}: {c['prec_min1_tgt']:,} prec / {c['proteins_tgt']:,} proteins (target, q<=0.01)")
    cols = ['dataset', 'version', 'prec_min1_tgt', 'prec_min1_unf', 'prec_min3_tgt', 'prec_min3_unf', 'proteins_tgt', 'proteins_unf', 'proteins_2pep_tgt', 'peptides_tgt', 'prot_avg_tgt', 'prot_complete_tgt']
...
_DEFAULT_DIANN_COUNTS = {'PXD003539': 6927, 'PXD030304': 9370, 'PXD004701': 6296, 'PXD017199': 10572, 'PXD041421': 9129}
DATASET_HEADLINES: dict[str, DatasetHeadline] = {'PXD003539': DatasetHeadline(paper_count=PXD003539_PAPER_COUNT, diann_count=_DEFAULT_DIANN_COUNTS['PXD003539'], paper_label='Guo 2019 (OpenSWATH)', metric='Protein groups (1% global FDR, target-only)'), 'PXD030304': DatasetHeadline(paper_count=PXD030304_PAPER_COUNT, diann_count=_DEFAULT_DIANN_COUNTS['PXD030304'], paper_label='ProCan 2022', metric='Proteins @ Global.Q.Value <= 0.01 (target-only)'), 'PXD004701': DatasetHeadline(paper_count=PXD004701_PAPER_COUNT, diann_count=_DEFAULT_DIANN_COUNTS['PXD004701'], paper_label='Sun 2023', metric='Proteins (consistency filter, target-only)'), 'PXD017199': DatasetHeadline(paper_count=0, diann_count=_DEFAULT_DIANN_COUNTS['PXD017199'], paper_label='', metric='Protein groups (1% global q-value, target-only)'), 'PXD041421': DatasetHeadline(paper_count=0, diann_count=_DEFAULT_DIANN_COUNTS['PXD041421'], paper_label='', metric='Protein groups (1% global q-value, target-only)')}
DATASET_HEADLINES_UNFILTERED: dict[str, int] = {ds: _DEFAULT_DIANN_COUNTS[ds] for ds in DATASET_HEADLINES}

def _compute_diann_counts(pg_matrix_path: Path | None, pr_matrix_path: Path | None=None) -> tuple[int, int]:
...
def refresh_dataset_headlines() -> None:
    ...
    sources: dict[str, tuple[Path | None, Path | None]] = {'PXD003539': (None, PXD003539_PR_MATRIX), 'PXD030304': (PXD030304_PG_MATRIX, None), 'PXD017199': (PXD017199_PG_MATRIX, None), 'PXD041421': (PXD041421_PG_MATRIX, None)}
    for ds, (pg, pr) in sources.items():
        unf, target = _compute_diann_counts(pg, pr)
        if target <= 0:
            continue
        h = DATASET_HEADLINES[ds]
        DATASET_HEADLINES[ds] = DatasetHeadline(paper_count=h.paper_count, diann_count=target, paper_label=h.paper_label, metric=h.metric)
    ...
    if PXD004701_PROTEIN_JSON.exists() and PXD004701_PROTEIN_JSON.stat().st_size > 0:
        ...
        if target_pgs:
            ...
            DATASET_HEADLINES['PXD004701'] = DatasetHeadline(paper_count=h.paper_count, diann_count=len(target_pgs), paper_label=h.paper_label, metric=h.metric)
    ...
    if PXD003539_PROTEIN_JSON.exists() and PXD003539_PROTEIN_JSON.stat().st_size > 0:
        ...
        if report_target > 0:
            ...
            DATASET_HEADLINES['PXD003539'] = DatasetHeadline(paper_count=h.paper_count, diann_count=report_target, paper_label=h.paper_label, metric=h.metric)
...
def _render_panel_a_headlines(ax, headlines: dict[str, DatasetHeadline]) -> None:
    ...
    bars_p = ax.bar(paper_drawn_xs, paper_drawn_vals, width=bar_width, color=COMPARISON['original'], label='Original analysis')
    bars_d = ax.bar(diann_xs, diann_vals_drawn, width=bar_width, color=COMPARISON['quantmsdiann'], label='quantmsdiann (DIA-NN)')
    ...
    for bar, v, d in zip(bars_d, diann_vals_drawn, datasets):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + pad, label, ha='center', va='bottom', fontsize=12, fontweight='bold', color=COMPARISON['quantmsdiann'])
    ax.set_xticks(x)
...
def _render_upset_in_axes(ax, sets: dict[str, set[str]], ds_order: list[str], *, bar_color: str=OKABE_ITO['blue'], panel_title: str | None=None, panel_letter: str | None=None, panel_subtitle: str | None=None) -> None:
...
def _render_panel_b_cellline_venn(ax, sets: dict[str, set[str]], region_sizes: dict[str, int] | None=None) -> None:
    ...
    _render_upset_in_axes(ax, sets, ds_order, bar_color=OKABE_ITO['blue'], panel_title='Cell-line set intersections (UpSet)')
...
def _render_panel_d_protein_venn(ax, sets: dict[str, set[str]], *, panel_letter: str | None=None, panel_subtitle: str | None=None) -> None:
    ...
    ds_order = [d for d in ('PXD003539', 'PXD030304', 'PXD004701', 'PXD017199', 'PXD041421') if d in sets]
    _render_upset_in_axes(ax, sets, ds_order, bar_color=OKABE_ITO['blue'], panel_letter=panel_letter, panel_subtitle=panel_subtitle)
...
def write_combined_counts_tsv(tsv_path: Path, headlines: dict[str, DatasetHeadline], cellline_sets: dict[str, set[str]], tissue_rows: list[tuple[str, dict[str, int]]], accession_sets: dict[str, set[str]], *, tissue_protein_rows: list[tuple[str, set[str]]] | list[tuple[str, dict[str, int]]] | None=None, tissue_protein_rows_per_cohort: list[tuple[str, dict[str, int]]] | None=None, runs_per_cohort: dict[str, int] | None=None) -> None:
    ...
    for ds in ds_order:
        ...
        rows.append((f'atlas_overlap | Panel B | {ds} | quantmsdiann', 'quantmsdiann (DIA-NN, target-only post-filter)', h.diann_count, h.metric))
        unfiltered = DATASET_HEADLINES_UNFILTERED.get(ds)
        if unfiltered is not None and unfiltered != h.diann_count:
            rows.append((f'atlas_overlap | Panel B | {ds} | quantmsdiann', 'quantmsdiann (DIA-NN, unfiltered pg_matrix/pr_matrix)', unfiltered, f'{h.metric} — pre conservative contaminant filter (2026-05-21 spec audit)'))
    for ds in ds_order:
    ...
    for ds in ds_order:
        ...
        if n_cl == 0 or n_acc == 0:
            continue
        rows.append((f'atlas_distribution | off-figure | breadth-vs-depth | {ds}', f'cell_lines={n_cl} runs={n_runs} accessions={n_acc}', n_acc, f'{ds}: {n_cl} cell line(s) × {n_runs} MS runs → {n_acc:,} target-only UniProt accessions'))
    for tissue, value in tissue_protein_rows or []:
        union_size = len(value) if isinstance(value, set) else sum(value.values())
        rows.append((f'atlas_distribution | Panel B | tissue proteins | {tissue}', 'union of UniProt accessions across all contributing cohorts', union_size, 'unique target-only proteins detected in this tissue across the atlas (post-contaminant filter)'))
    for tissue, by_ds in tissue_protein_rows_per_cohort or []:
        for ds in ds_order:
            rows.append((f'atlas_distribution | Panel B | tissue proteins (per-cohort breakdown) | {tissue}', ds, by_ds.get(ds, 0), 'per-cohort target-only protein count for this tissue (sum across cohorts double-counts proteins seen in multiple cohorts — figure shows union)'))
...
def _figure_combined_cell_lines_atlas__main() -> int:
    ...
    print('Refreshing DATASET_HEADLINES diann_count from pg_matrix files (target-only contaminant filter)...')
    refresh_dataset_headlines()
    for ds, h in DATASET_HEADLINES.items():
        unf = DATASET_HEADLINES_UNFILTERED.get(ds)
        print(f'  {ds}: target_only={h.diann_count:,}  unfiltered={unf:,}  (delta {unf - h.diann_count:,})')
    ...
    panel_a_headlines = {'PXD003539': DatasetHeadline(4284, 6592, 'Guo 2019 (OpenSWATH)', 'Protein groups (>=2 peptides)'), 'PXD030304': DatasetHeadline(6692, 8645, 'ProCan 2022', 'Protein groups (>=2 peptides)')}
    TOP_TISSUES = 12
...
def _figure_fig2_validation__render(out: Path) -> Path:
    ...
    draw_strip(ax[2], compact=True, dataset_colors=ds_colors)
...
def _figure_fig2_validation__main() -> int:
...
def count_matrix_data_rows_split(matrix_path: Path) -> tuple[int, int]:
    ...
    try:
        df = pd.read_csv(matrix_path, sep='\t', usecols=['Protein.Group'], dtype=str)
    except (FileNotFoundError, OSError, ValueError):
...
def count_pr_matrix_min_replicates(matrix_path: Path, min_replicates: int) -> int:
    ...
    """
    _unf, target = count_pr_matrix_min_replicates_split(matrix_path, min_replicates)
    return target

def count_pr_matrix_min_replicates_split(matrix_path: Path, min_replicates: int) -> tuple[int, int]:
    ...
    metadata = {'Protein.Group', 'Protein.Ids', 'Protein.Names', 'Genes', 'First.Protein.Description', 'Proteotypic', 'Stripped.Sequence', 'Modified.Sequence', 'Precursor.Charge', 'Precursor.Id'}
    ...
    if 'Protein.Group' not in df.columns:
        return (unfiltered, unfiltered)
    target_mask = passes & df['Protein.Group'].map(lambda v: is_target_protein_group(v) if isinstance(v, str) else False)
...
_VERSION_COLORS = dict(VERSION_COLORS)
...
def load_report_counts() -> dict[tuple[str, str], dict]:
    ...
    for _, r in df.iterrows():
        out[r['dataset'], r['version']] = {k: int(r[k]) for k in ('prec_min1_tgt', 'prec_min1_unf', 'prec_min3_tgt', 'prec_min3_unf', 'proteins_tgt', 'proteins_unf', 'prot_avg_tgt', 'prot_complete_tgt')}
    return out

def _figure_quantmsdiann_benchmarks_vs_proteobench__main() -> int:
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for dataset in _figure_quantmsdiann_benchmarks_vs_proteobench__DATASET_TO_MODULE:
        for version in DIANN_VERSIONS:
            ...
            precursors_min3, precursors_min3_unf = (c['prec_min3_tgt'], c['prec_min3_unf'])
            ...
            quantmsdiann_rows_unfiltered.append((dataset, version, precursors_unf, proteins_unf))
            quantmsdiann_rows_min3_unfiltered.append((dataset, version, precursors_min3_unf, proteins_unf))
            quantmsdiann_rows_protavg.append((dataset, version, precursors, c['prot_avg_tgt']))
            quantmsdiann_rows_complete.append((dataset, version, precursors, c['prot_complete_tgt']))
            print(f'{dataset} {version}: precursors_min1={precursors:,} (unfiltered={precursors_unf:,})  precursors_min3={precursors_min3:,} (unfiltered={precursors_min3_unf:,})  proteins={proteins:,} (unfiltered={proteins_unf:,})  [report, run+global q<=0.01]')
...
def _figure_id_vs_epsilon__main() -> int:
...
def _figure_mdc_cluster_runtime__main() -> int:
...
def render_figure(counts: _figure_original_vs_quantmsdiann__Counts, svg_path: Path) -> None:
...
def render_missing_values_per_run(guo_per_run: dict[str, float], diann_per_run: dict[str, float], svg_path: Path) -> None:
    ...
    x = range(len(common))
    ax.plot(list(x), [guo_by_stem[s] for s in common], label='Guo 2019 (OpenSWATH)', color=COMPARISON['original'], linewidth=1.0)
    ax.plot(list(x), [diann_by_stem[s] for s in common], label='quantmsdiann (DIA-NN)', color=COMPARISON['quantmsdiann'], linewidth=1.0)
...
def render_genes_per_condition(walzer_per_cond: dict[str, set[str]], diann_per_cond: dict[str, set[str]], svg_path: Path) -> None:
    ...
    bars_d = ax.bar([xi + bar_width / 2 for xi in x], diann_vals, width=bar_width, color=COMPARISON['quantmsdiann'], label='quantmsdiann (DIA-NN)')
...
def render_peptides_per_protein(guo_peptide_counts: dict[str, int], diann_peptide_counts: dict[str, int], svg_path: Path, thresholds: tuple[int, ...]=(1, 2, 3, 5, 10)) -> None:
    ...
    bars_guo = ax.bar([xi - width / 2 for xi in x], guo_values, width=width, color=COMPARISON['original'], label='Guo 2019 (OpenSWATH)')
    bars_diann = ax.bar([xi + width / 2 for xi in x], diann_values, width=width, color=COMPARISON['quantmsdiann'], label='quantmsdiann (DIA-NN)')
...
def _figure_original_vs_quantmsdiann__main() -> int:
    ...
    print(f'  pr_matrix unique Protein.Group: unfiltered={pg_unf:,}  target_only={pg_target:,} (audit only)')
    import json as _json
    with open(_figure_original_vs_quantmsdiann__DATA_DIR / 'diann_report_protein_counts.json', encoding='utf-8') as _fh:
...
def collect_runtime_rows(*, fetch: bool=True) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows)
INSTRUMENT_COLOURS = dict(INSTRUMENT_COLORS)
...
def render_per_step_boxplot(durations: dict[str, list[float]], summary: pd.DataFrame, svg_path: Path, fig_h: float | None=None) -> None:
    ...
    bp = ax.boxplot(data, vert=False, whis=(5, 95), showfliers=True, flierprops=dict(marker='o', markerfacecolor='#888888', markeredgecolor='none', markersize=3.5, alpha=0.6), medianprops=dict(color=OKABE_ITO['vermillion'], linewidth=1.6), boxprops=dict(color=OKABE_ITO['blue'], linewidth=1.2), whiskerprops=dict(color=OKABE_ITO['blue'], linewidth=1.0), capprops=dict(color=OKABE_ITO['blue'], linewidth=1.0), patch_artist=False)
    del bp
...
def render_resources_boxplot(resources: dict[str, dict[str, list[float]]], summary: pd.DataFrame, svg_path: Path) -> None:
    ...
    _box_kw = dict(medianprops=dict(color=OKABE_ITO['vermillion'], linewidth=1.4), boxprops=dict(color=OKABE_ITO['blue']), whiskerprops=dict(color=OKABE_ITO['blue']), capprops=dict(color=OKABE_ITO['blue']))
    ax_rss.boxplot(rss_data, vert=False, widths=0.62, labels=steps, showfliers=False, **_box_kw)
    ...
    ax_cpu.boxplot(cpu_data, vert=False, widths=0.62, labels=steps, showfliers=False, **_box_kw)
    ax_cpu.set_xlabel('Threading efficiency (%)', fontsize=10)
    ax_cpu.axvline(100.0, color=OKABE_ITO['bluish_green'], linestyle='--', linewidth=0.9, zorder=1)
...
def _figure_performance_trace__main() -> int:
...
_figure_phospho__VCOL = {v: VERSION_COLORS[v] for v in _figure_phospho__VERSIONS}
...
def _deposited_panel(ax):
    ...
    cols = [COMPARISON['original'], COMPARISON['quantmsdiann']]
    ...
    ax.annotate(f'+{pct}%', (bars[1].get_x() + bars[1].get_width() / 2, vals[1]), textcoords='offset points', xytext=(0, 16), ha='center', va='bottom', fontsize=9, fontweight='bold', color=COMPARISON['quantmsdiann'])
    ax.set_xticks([0, 1])
...
def _figure_phospho__render(out: Path) -> Path:
    ...
    handles += [Line2D([0], [0], marker='s', linestyle='none', markersize=9, markerfacecolor=COMPARISON['original'], markeredgecolor='white', label='deposited (original)'), Line2D([0], [0], marker='s', linestyle='none', markersize=9, markerfacecolor=COMPARISON['quantmsdiann'], markeredgecolor='white', label='quantms.io reanalysis')]
...
matplotlib.use('Agg')
apply_house_style()
_figure_proteobench_accuracy__REPO_ROOT = Path(__file__).resolve().parents[1]
COMMUNITY_DIR = _figure_proteobench_accuracy__REPO_ROOT / 'data' / 'quantmsdiann_benchmarks' / 'proteobench'
_figure_proteobench_accuracy__OUT = _figure_proteobench_accuracy__REPO_ROOT / 'analysis' / 'figures' / 'manuscript' / 'fig1d_proteobench_accuracy.svg'
SPECIES = ('HUMAN', 'YEAST', 'ECOLI')
GREY = OKABE_ITO['grey']
_figure_proteobench_accuracy__VERSIONS = ('v1_8_1', 'v2_5_1_enterprise')
...
def draw(ax, threshold: int=3, *, with_legend: bool=True, compact: bool=False, square: bool=True) -> None:
    ...
    for ds in datasets:
        ...
        qm = qm[qm['version'].isin(_figure_proteobench_accuracy__VERSIONS)]
        for sp in SPECIES:
    ...
    if with_legend:
        ...
        for ver in _figure_proteobench_accuracy__VERSIONS:
    ...
    for x, sp in enumerate(order):
        ...
        for di, ds in enumerate(datasets):
            ...
            for _, r in qm.iterrows():
                ver = r['version']
                ver_dx = -0.13 if ver == _figure_proteobench_accuracy__VERSIONS[0] else 0.13
...
def _figure_proteobench_accuracy__render(out: Path, threshold: int=3) -> Path:
...
def _figure_proteobench_accuracy__main() -> int:
    print(f'wrote {_figure_proteobench_accuracy__render(_figure_proteobench_accuracy__OUT)}')
    return 0
...
class _figure_pxd004701_sun_vs_quantmsdiann__Counts:
    ...
    quantmsdiann_proteins_consistent_unfiltered: int
...
def _figure_pxd004701_sun_vs_quantmsdiann__render_main_figure(counts: _figure_pxd004701_sun_vs_quantmsdiann__Counts, svg_path: Path) -> None:
    ...
    bars_s = ax.bar([xi - bar_width / 2 for xi in x], sun_vals, width=bar_width, color=COMPARISON['original'], label='Sun et al. 2023')
    bars_d = ax.bar([xi + bar_width / 2 for xi in x], diann_vals, width=bar_width, color=COMPARISON['quantmsdiann'], label='quantmsdiann (DIA-NN)')
...
def render_proteins_per_subtype(diann_per_subtype: dict[str, set[str]], svg_path: Path, *, sun_reference: int=SUN_PROTEINS) -> None:
    ...
    ax.axhline(sun_reference, color=COMPARISON['original'], linestyle='--', linewidth=1, label=f'Sun et al. 2023 global ({sun_reference:,})')
    ax.set_xticks(x)
...
def _figure_pxd004701_sun_vs_quantmsdiann__write_counts_tsv(counts: _figure_pxd004701_sun_vs_quantmsdiann__Counts, tsv_path: Path, *, diann_per_subtype: dict[str, set[str]] | None=None) -> None:
    """Auditable counts table with inline methodology notes."""
    rows = [('Protein groups (consistency filter)', 'Sun et al. 2023 (paper headline)', counts.sun_proteins, '6,091 SwissProt proteins; proteotypic, Global.Q.Value<=0.01, >=10% detection across samples (paper, Methods)'), ('Protein groups (consistency filter)', 'quantmsdiann (DIA-NN, target-only)', counts.quantmsdiann_proteins_consistent, 'post-consistency-filter union; conservative contaminant filter (CONTAM_/Cont_/ENTRAP_/DECOY_/decoy_) applied at Protein.Group level per 2026-05-21 spec'), ('Protein groups (consistency filter)', 'quantmsdiann (DIA-NN, unfiltered)', counts.quantmsdiann_proteins_consistent_unfiltered, 'audit baseline: same consistency-filter union BEFORE the contaminant filter; includes CONTAM_/ENTRAP_ rows'), ('Protein groups (1% FDR, no consistency)', 'Sun et al. 2023 (paper pre-filter)', counts.sun_proteins_raw, '8,952 proteins identified pre-consistency-filter (paper, Methods)'), ('Protein groups (1% FDR, no consistency)', 'quantmsdiann (DIA-NN, target-only)', counts.quantmsdiann_proteins_strict, "report-based: unique Protein.Group at Global.PG.Q.Value<=0.01 in diann_report.parquet (DIA-NN 2.5.1, --qvalue 0.05), contaminant/entrapment/decoy stripped; comparable to Sun's 8,952"), ('Protein groups (1% FDR, no consistency)', 'quantmsdiann (DIA-NN, unfiltered pg_matrix)', counts.quantmsdiann_proteins_pg_matrix_unfiltered, 'raw row count of diann_report.pg_matrix.tsv, pre-filter'), ('Protein groups (1% FDR, no consistency)', 'quantmsdiann (DIA-NN, diannsummary.log)', counts.quantmsdiann_proteins_strict_unfiltered, 'audit baseline: diannsummary.log unfiltered headline'), ('Proteotypic peptides', 'Sun et al. 2023 (paper headline)', counts.sun_peptides, '90,762 proteotypic peptides under the same consistency filter (paper, Methods)'), ('Proteotypic peptides', 'quantmsdiann (DIA-NN)', counts.quantmsdiann_peptides, 'unique Stripped.Sequence in pr_matrix.tsv among rows with >=1 non-NA sample'), ('Precursors', 'quantmsdiann (DIA-NN, 1% FDR)', counts.quantmsdiann_precursors, 'from diannsummary.log (Target precursors at 1% global q-value)'), ('Spectral library precursors', 'Sun et al. 2023 (pan-human CAL)', SUN_LIBRARY_PRECURSORS, '194,899 library precursors; NOT identified precursors (PXD009597)'), ('Spectral library proteins', 'Sun et al. 2023 (pan-human CAL)', SUN_LIBRARY_PROTEINS, '10,323 SwissProt library proteins (PXD009597)'), ('Cell lines: TNBC', 'Sun et al. 2023 (paper-reported split)', counts.sun_tnbc, "39 TNBC lines in paper's split of the 76 cell lines"), ('Cell lines: non-TNBC', 'Sun et al. 2023 (paper-reported split)', counts.sun_non_tnbc, "37 non-TNBC lines in paper's split of the 76 cell lines")]
...
def _figure_pxd004701_sun_vs_quantmsdiann__main() -> int:
    _figure_pxd004701_sun_vs_quantmsdiann__DATA_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print(f'  pg_matrix rows: unfiltered={pg_unf:,}  target_only={pg_target:,} (delta {pg_unf - pg_target:,})')
    ...
    print(f'  report protein groups (Global.PG.Q.Value<=0.01): target={report_proteins_target:,} unfiltered={report_proteins_unfiltered:,}')
    ...
    for s, pgs in diann_per_subtype.items():
        ...
        diann_proteins_consistent.update((pg_str for pg_str in pgs if is_target_protein_group(pg_str)))
        print(f'  {s:<14s} {len(pgs):>6,}')
    print(f'  union across subtypes (unfiltered): {len(diann_proteins_consistent_unf):,}')
    print(f'  union across subtypes (target_only): {len(diann_proteins_consistent):,}')
    counts = _figure_pxd004701_sun_vs_quantmsdiann__Counts(sun_proteins=SUN_PROTEINS, sun_proteins_raw=SUN_PROTEINS_RAW, sun_peptides=SUN_PEPTIDES, sun_tnbc=SUN_TNBC, sun_non_tnbc=SUN_NON_TNBC, quantmsdiann_proteins_strict=report_proteins_target, quantmsdiann_proteins_strict_unfiltered=report_proteins_unfiltered, quantmsdiann_proteins_pg_matrix_unfiltered=pg_unf, quantmsdiann_proteins_consistent=len(diann_proteins_consistent), quantmsdiann_proteins_consistent_unfiltered=len(diann_proteins_consistent_unf), quantmsdiann_peptides=pep, quantmsdiann_precursors=prec)
...
def _figure_pxd030304_procan_vs_quantmsdiann__render_main_figure(counts: _figure_pxd030304_procan_vs_quantmsdiann__Counts, svg_path: Path) -> None:
    ...
    bar_width = 0.35
    x = list(range(len(metrics)))
    bars_p = ax.bar([xi - bar_width / 2 for xi in x], procan_vals, width=bar_width, color=COMPARISON['original'], label='ProCan-DepMapSanger 2022')
    bars_d = ax.bar([xi + bar_width / 2 for xi in x], diann_vals, width=bar_width, color=COMPARISON['quantmsdiann'], label='quantmsdiann (DIA-NN)')
...
def _figure_pxd030304_procan_vs_quantmsdiann__render_per_run_completeness(procan_freqs: list[float], diann_freqs: list[float], svg_path: Path) -> None:
    ...
    ax.plot(x, procan_curve, color=COMPARISON['original'], linewidth=1.8, label=f'ProCan-DepMapSanger 2022 ({len(procan):,} protein groups)')
    ax.plot(x, diann_curve, color=COMPARISON['quantmsdiann'], linewidth=1.8, label=f'quantmsdiann (DIA-NN) ({len(diann):,} protein groups)')
...
def _figure_pxd030304_procan_vs_quantmsdiann__write_counts_tsv(counts: _figure_pxd030304_procan_vs_quantmsdiann__Counts, tsv_path: Path, *, procan_per_tissue: dict[str, set[str]] | None=None, diann_per_tissue: dict[str, set[str]] | None=None) -> None:
    ...
    machine-readable."""
    rows = [('Protein groups', 'ProCan-DepMapSanger 2022 (paper headline)', counts.procan_proteins, '8,498 proteins quantified at Global.Q.Value <= 0.01 (paper, Results)'), ('Protein groups (>=2 peptides)', 'ProCan-DepMapSanger 2022 (stringent)', counts.procan_proteins_stringent, '6,692 proteins with >=2 supporting peptides (paper, Results)'), ('Protein groups', 'quantmsdiann (DIA-NN, 1% FDR, target-only)', counts.quantmsdiann_proteins, 'post-filter: pg_matrix.tsv rows whose Protein.Group has no CONTAM_/Cont_/ENTRAP_/DECOY_/decoy_ token (conservative filter, 2026-05-21 spec)'), ('Protein groups', 'quantmsdiann (DIA-NN, 1% FDR, unfiltered pg_matrix)', counts.quantmsdiann_proteins_pg_matrix_unfiltered, 'raw row count of diann_report.pg_matrix.tsv, pre-filter; includes CONTAM_/ENTRAP_/DECOY_ rows'), ('Protein groups', 'quantmsdiann (DIA-NN, 1% FDR, diannsummary.log)', counts.quantmsdiann_proteins_unfiltered, "audit baseline: diannsummary.log 'Protein groups with global q-value <= 0.01' line (unfiltered)"), ('Protein groups (>=2 peptides)', 'quantmsdiann (DIA-NN, 1% FDR)', counts.quantmsdiann_proteins_stringent, '>=2 unique Stripped.Sequence per Protein.Group (proteotypic) in pr_matrix.tsv'), ('Precursors', 'quantmsdiann (DIA-NN, 1% FDR)', counts.quantmsdiann_precursors, 'from diannsummary.log (Target precursors at 1% global q-value)'), ('Spectral library precursors', 'ProCan-DepMapSanger 2022', PROCAN_LIBRARY_PRECURSORS, 'library size (paper, STAR Methods); NOT identified precursors'), ('Spectral library proteins', 'ProCan-DepMapSanger 2022', PROCAN_LIBRARY_PROTEINS, 'library protein count (paper, STAR Methods)'), ('MS runs', 'ProCan-DepMapSanger 2022', PROCAN_MS_RUNS, 'paper headline (PRIDE archive lists 6,981)')]
    if procan_per_tissue is not None or diann_per_tissue is not None:
        ...
        note_diann = "per-tissue union of Protein.Group from diann_report.parquet filtered to Proteotypic == 1 AND Global.Q.Value <= 0.01 (ProCan's filter applied to the long-format report; no per-cell quant FDR)"
        for t in tissues:
...
def _figure_pxd030304_procan_vs_quantmsdiann__main() -> int:
    ...
    counts = _figure_pxd030304_procan_vs_quantmsdiann__Counts(procan_proteins=PROCAN_PROTEINS, procan_proteins_stringent=PROCAN_PROTEINS_STRINGENT, quantmsdiann_proteins=report_proteins, quantmsdiann_proteins_unfiltered=int(_rep['proteins_global_unf']), quantmsdiann_proteins_pg_matrix_unfiltered=pg_unf, quantmsdiann_proteins_stringent=diann_stringent, quantmsdiann_precursors=prec)
...
def _qm_counts_parquet() -> tuple[int, int]:
    ...
    import pyarrow.parquet as pq
    parq = _download(f'{_QB}/diann_report.parquet', _figure_pxd064049_spatial_vs_quantmsdiann__CACHE_DIR / 'qm_report.parquet')
    cols = ['Precursor.Id', 'Protein.Group', 'Q.Value', 'Global.Q.Value', 'Global.PG.Q.Value', 'Decoy']
...
def render_main_comparison(or_pr: int, qm_pr: int, or_pg: int, qm_pg: int, svg_path: Path) -> None:
    ...
    conditions = [('Original (DIA-NN 1.8.1)', _figure_pxd064049_spatial_vs_quantmsdiann__ORIG_COLOUR, or_pr, or_pg), ('quantmsdiann (DIA-NN 2.5.1-enterprise)', QM_COLOUR, qm_pr, qm_pg)]
...
def _figure_pxd064049_spatial_vs_quantmsdiann__main() -> int:
...
def _figure_queue_size_sweep__main() -> int:
...
def _figure_reanalysis_improvement__main() -> int:
...
_figure_single_cell_combined__VCOL = {v: VERSION_COLORS[v] for v in VERS}
_figure_single_cell_combined__ACC = {'HeLa Astral SC': 'PXD046357', 'HeLa One-Tip': 'PXD044991'}
ACC_SHORT = {'Astral': 'PXD046357', 'One-Tip': 'PXD044991'}
_figure_single_cell_combined__FLAG = 'HeLa Astral SC'
FLAG_T = f'HeLa Astral SC\n({_figure_single_cell_combined__ACC[_figure_single_cell_combined__FLAG]})'
PLEX_COUNTS = _figure_single_cell_combined__REPO / 'analysis' / 'figures' / 'plexDIA' / 'MSV000093870' / 'comparison_counts.tsv'
...
def _completeness(ax):
    ...
    for ds in df['dataset'].unique():
        for v in VERS:
            ...
            ax.plot(s['min_cells'], s['n_proteins'], linestyle=DS_STYLE.get(ds, '-'), marker='o', ms=3.5, lw=1.8, color=_figure_single_cell_combined__VCOL[v])
...
def _percell(ax):
    ...
    for gi, ds in enumerate(datasets):
        for k, v in enumerate(VERS):
            ...
            style_boxplot(bp, color=_figure_single_cell_combined__VCOL[v])
            ax.scatter(x + rng.uniform(-0.07, 0.07, len(vals)), vals, s=12, color=_figure_single_cell_combined__VCOL[v], alpha=0.7, edgecolors='none', zorder=3)
    ax.set_xticks(range(len(datasets)))
    ax.set_xticklabels([f'{d}\n({_figure_single_cell_combined__ACC[d]})' for d in datasets])
    ax.set_ylabel('protein groups / cell')
    ax.set_title('Per-cell protein groups')
    kfmt_axis(ax.yaxis)
    despine(ax)

def _plexdia(ax):
    plex_dep, plex_qm = _load_plex()
    vals = [plex_dep, plex_qm]
    cols = [COMPARISON['original'], COMPARISON['quantmsdiann']]
    bw = 0.38
    pos = [-bw / 2, bw / 2]
    b = ax.bar(pos, vals, bw, color=cols, edgecolor='white', linewidth=0.6)
    pct = round(100 * (vals[1] - vals[0]) / vals[0])
    ax.annotate(f'+{pct}%', (b[1].get_x() + b[1].get_width() / 2, vals[1]), textcoords='offset points', xytext=(0, 3), ha='center', va='bottom', fontsize=12, fontweight='bold', color=COMPARISON['quantmsdiann'])
    ax.set_xticks(pos)
    ax.set_xticklabels(['Galatidou 2024\n(deposited)\n1.8.1b16', 'quantms.io\n2.5.0'], fontsize=11)
    ax.set_xlim(-0.6, 0.6)
    ax.set_ylabel('protein groups')
    ax.set_title('plexDIA — MSV000093870')
    kfmt_axis(ax.yaxis)
    despine(ax)
...
def _rank(ax):
    ...
    for v in VERS:
        ...
        y = s['log10_intensity'].values - s['log10_intensity'].max()
        ax.plot(s['rank'], y, '-', color=_figure_single_cell_combined__VCOL[v], label=VLAB[v])
    ax.set_xlabel('protein group rank')
    ax.set_ylabel('log10 intensity (rel. to top)')
...
def _totals(ax):
    ...
    def draw(target, xmap, metric):
        for k, v in enumerate(VERS):
            ...
            for d in dsx:
                ...
                target.bar(x, hi, bw, color=_figure_single_cell_combined__VCOL[v], edgecolor='white', linewidth=0.6)
                if v != '1_8_1':
                    lo = TOTALS[d][metric][0]
                    target.annotate(f'+{round(100 * (hi - lo) / lo)}%', (x, hi), textcoords='offset points', xytext=(0, 3), ha='center', va='bottom', fontsize=10, fontweight='bold', color=_figure_single_cell_combined__VCOL[v])
    ...
    ax.text(np.mean([prec_x[d] for d in dsx]), -0.2, 'precursors', transform=ax.get_xaxis_transform(), ha='center', va='top', fontsize=11, fontweight='bold')
    ax.text(np.mean([prot_x[d] for d in dsx]), -0.2, 'protein groups', transform=ax.get_xaxis_transform(), ha='center', va='top', fontsize=11, fontweight='bold')
    for sp in ('top',):
        ax.spines[sp].set_visible(False)
    ...
    for k, v in enumerate(VERS):
        ...
        for d in dsx:
            ...
            if v != '1_8_1':
                ...
                ax.annotate(f'+{round(100 * (hp - lo) / lo)}%', (xp, hp), textcoords='offset points', xytext=(0, 3), ha='center', va='bottom', fontsize=9, fontweight='bold', color=_figure_single_cell_combined__VCOL[v])
            ...
            if v != '1_8_1':
                ...
                ax2.annotate(f'+{round(100 * (hg - lo) / lo)}%', (xt, hg), textcoords='offset points', xytext=(0, 3), ha='center', va='bottom', fontsize=9, fontweight='bold', color=_figure_single_cell_combined__VCOL[v])
...
def _figure_single_cell_combined__render(out: Path) -> Path:
    ...
    handles += [Line2D([0], [0], color='#555555', linestyle='-', linewidth=2, label='PXD046357 (HeLa Astral)'), Line2D([0], [0], color='#555555', linestyle='--', linewidth=2, label='PXD044991 (One-Tip)')]
...
def _figure_single_cell_combined__main() -> int:
...
def _count(ftp_dir: str, version: str) -> tuple[int, int, int]:
    ...
    r = pq.read_table(rep, columns=['Modified.Sequence', 'Q.Value', 'Global.Q.Value', 'Protein.Group']).to_pandas()
...
def _make_phospho_tables__main() -> int:
    rows = []
...
_make_single_cell_tables__ACC = {'HeLa Astral SC': 'PXD046357', 'HeLa One-Tip': 'PXD044991'}
...
_COLS = ['Run', 'Precursor.Id', 'Protein.Group', 'Q.Value', 'Global.Q.Value', 'Global.PG.Q.Value', 'PG.Q.Value', 'PG.MaxLFQ', 'Decoy']
...
def _make_single_cell_tables__main() -> int:
...
def _venn_protein_accessions__main() -> int:
...
# ===========================================================================
# ORCHESTRATOR -- the rebuild CLI (formerly scripts/rebuild.py)
# ===========================================================================
...
def recompute_report_counts() -> int:
    ...
    FTP and recount under the methods.md section 1 rule -> report_counts.tsv. The
    counting primitive (count_report) and download_if_missing are inlined above."""
...
def recompute_reanalysis_pg_counts() -> int:
    ...
    prot_2pep at Lib.Q.Value) under methods.md section 1. Decoys dropped, nothing
    else filtered. Consumed by the PXD004701 / PXD030304 figure scripts."""
    reports = {
...
# Stage = (name, target, description). target is one of:
#   * an in-process callable returning an int return code (inlined figure mains
#     and the two recompute steps), or
#   * a module name string (run as `python -m <module>`) for the plexDIA scripts
#     and the paper-numbers collector, which were never part of the merge.
...
from pathlib import Path
import pandas as pd
_cpn_REPO = Path(__file__).resolve().parents[1]
OUT_TSV = _cpn_REPO / 'data' / 'paper_numbers.tsv'
OUT_TEX = _cpn_REPO / 'paper' / 'generated_numbers.tex'
...
def _read(path: Path) -> pd.DataFrame | None:
    if not path.exists():
        print(f'WARN: missing {path.relative_to(_cpn_REPO)}', file=sys.stderr)
        return None
    return pd.read_csv(path, sep='\t')

def collect_benchmarks(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'data' / 'quantmsdiann_benchmarks' / 'report_counts.tsv')
    if df is None:
        return
...
def collect_reanalysis(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'analysis' / 'figures' / 'reanalysis' / 'data' / 'reanalysis_improvement.tsv')
    if df is None:
        return
    for _, r in df.iterrows():
        ds = str(r['dataset'])
...
def collect_single_cell(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'data' / 'single_cell' / 'sc_totals.tsv')
    if df is not None:
        for _, r in df.iterrows():
    ...
    pc = _read(_cpn_REPO / 'data' / 'single_cell' / 'mv_per_cell.tsv')
...
def collect_plexdia(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'analysis' / 'figures' / 'plexDIA' / 'MSV000093870' / 'comparison_counts.tsv')
    if df is None:
        return
...
def collect_phospho(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'data' / 'phospho' / 'phospho_counts.tsv')
    if df is None:
        return
    for _, r in df.iterrows():
        tag = f"{r['dataset']} {r['version']}"
...
def collect_atlas(N: Numbers) -> None:
    df = _read(_cpn_REPO / 'analysis' / 'figures' / 'combined' / 'data' / 'combined_counts.tsv')
    if df is None:
        return
    for _, r in df.iterrows():
...
def _cpn_main() -> int:
    ...
    print(f'wrote {OUT_TSV.relative_to(_cpn_REPO)} ({len(N.rows)} numbers) and {OUT_TEX.relative_to(_cpn_REPO)} ({n_macros} macros)')
    return 0
...
import pandas as pd
import pyarrow.parquet as pq
_plex_REPO_ROOT = Path(__file__).resolve().parent.parent
_plex_FIGURES_DIR = _plex_REPO_ROOT / 'analysis' / 'figures' / 'plexDIA' / 'MSV000093870'
_plex_CACHE_DIR = _plex_FIGURES_DIR / 'data' / 'cache'
FTP_BASE = 'https://ftp.pride.ebi.ac.uk/pub/databases/pride/resources/proteomes/quantmsdiann-benchmarks/single-cell/MSV000093870/v2_5_1'
...
def _plex_oocyte_main() -> int:
    report = _plex_cached_report()
    confident = load_channel_confident(report)
    cells = per_cell_counts(confident)
    _plex_FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    render_per_cell_figure(cells, _plex_FIGURES_DIR / 'main_plexdia_per_cell.svg')
    write_counts(cells, confident, _plex_FIGURES_DIR / 'counts.tsv')
    ...
    print(f"Wrote {_plex_FIGURES_DIR / 'main_plexdia_per_cell.svg'}")
    print(f"Wrote {_plex_FIGURES_DIR / 'counts.tsv'}")
    return 0
...
def _cached_original() -> Path:
    _plex_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    dest = _plex_CACHE_DIR / 'Galatidou_2024_ProteinsXcells.csv'
    if dest.exists() and dest.stat().st_size > 0:
        return dest
...
def render_qc_matched(matched: pd.DataFrame, svg_path: Path) -> None:
    ...
    ax = axes[0]
    ax.scatter(matched['orig_proteins'], matched['qm_proteins'], s=22, c=_galat_QM_COLOUR, alpha=0.7, edgecolors='#37474f', linewidths=0.5)
    lo = 0
    ...
    bp = ax.boxplot(data, widths=0.6, patch_artist=True, showfliers=False, medianprops=dict(color='#212121', linewidth=1.4))
    for patch, c in zip(bp['boxes'], (ORIG_COLOUR, _galat_QM_COLOUR)):
...
def render_comparison(orig_per_cell: pd.Series, qm_cells: pd.DataFrame, orig_total: int, qm_total: int, shared: int, orig_only: int, qm_only: int, svg_path: Path) -> None:
    ...
    for patch, c in zip(bp['boxes'], (ORIG_COLOUR, _galat_QM_COLOUR)):
        ...
        patch.set_edgecolor('#37474f')
    rng = np.random.default_rng(0)
    for i, (vals, c) in enumerate(zip(data, (ORIG_COLOUR, _galat_QM_COLOUR)), start=1):
        ax.scatter(rng.normal(i, 0.05, size=len(vals)), vals, s=10, c='#37474f', alpha=0.4, linewidths=0, zorder=3)
    ax.set_xticks([1, 2])
    ...
    ax = axes[1]
    bars = ax.bar([0, 1], [orig_total, qm_total], width=0.6, color=[ORIG_COLOUR, _galat_QM_COLOUR], edgecolor='#37474f')
    ...
    cols = ['#43a047', ORIG_COLOUR, _galat_QM_COLOUR]
...
def render_total_pg(orig_total: int, qm_total: int, svg_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(7, 5))
    bars = ax.bar([0, 1], [orig_total, qm_total], width=0.55, color=[ORIG_COLOUR, _galat_QM_COLOUR], edgecolor='#37474f')
    ax.set_xlim(-0.7, 1.7)
    for b, v in zip(bars, (orig_total, qm_total)):
...
def _galat_main() -> int:
    confident = load_channel_confident(_plex_cached_report())
    ...
    _plex_FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    render_comparison(orig_per_cell, qm_cells, orig_total=len(orig_acc), qm_total=qm_total, shared=shared, orig_only=orig_only, qm_only=qm_only, svg_path=_plex_FIGURES_DIR / 'main_galatidou_comparison.svg')
    render_total_pg(orig_total=len(orig_acc), qm_total=qm_total, svg_path=_plex_FIGURES_DIR / 'main_galatidou_total_pg.svg')
    matched = qc_matched_table(matrix_path, qm_cells)
    r = render_qc_matched(matched, _plex_FIGURES_DIR / 'main_galatidou_qc_matched.svg')
    counts = _plex_FIGURES_DIR / 'comparison_counts.tsv'
    ...
    print(f"Wrote {_plex_FIGURES_DIR / 'main_galatidou_comparison.svg'}")
    print(f"Wrote {_plex_FIGURES_DIR / 'main_galatidou_qc_matched.svg'}")
...
DATA_PREP = [
    ...
    ("single_cell_tables", _make_single_cell_tables__main,
     "Per-cell / completeness / CV tables for the single-cell figure"),
    ("phospho_tables", _make_phospho_tables__main,
     "Phosphopeptide / phosphosite tables (Lib.Q.Value; site Prob>=0.99)"),
]

FIGURES = [
    ("benchmarks", _figure_quantmsdiann_benchmarks_vs_proteobench__main,
     "Fig 2 -- ProteoBench benchmark panels + counts.tsv"),
    ("queue_sweep", _figure_queue_size_sweep__main,
     "queue_size_sweep.tsv (consumed by the Fig 2 validation composite)"),
    ("fig2_validation", _figure_fig2_validation__main,
     "Fig 2 validation composite"),
    ("id_vs_epsilon", _figure_id_vs_epsilon__main,
     "ProteoBench id-vs-epsilon panel"),
    ("proteobench_accuracy", _figure_proteobench_accuracy__main,
     "ProteoBench accuracy panels"),
    ("reanalysis_improvement", _figure_reanalysis_improvement__main,
     "Reanalysis-recovery figure (original vs quantmsdiann)"),
    ("single_cell_combined", _figure_single_cell_combined__main,
     "Single-cell figure (per-cell depth, completeness, CV)"),
    ("plexdia_per_cell", _plex_oocyte_main,
     "plexDIA per-cell depth (MSV000093870)"),
    ("plexdia_vs_galatidou", _galat_main,
     "plexDIA deposited vs quantmsdiann"),
    ("pxd003539", _figure_original_vs_quantmsdiann__main,
     "PXD003539 (NCI-60) panels"),
    ("pxd004701", _figure_pxd004701_sun_vs_quantmsdiann__main,
     "PXD004701 (Sun) panels"),
    ("pxd030304", _figure_pxd030304_procan_vs_quantmsdiann__main,
     "PXD030304 (ProCan) panels -- streams a 2GB matrix"),
    ("pxd064049_spatial", _figure_pxd064049_spatial_vs_quantmsdiann__main,
     "PXD064049 spatial DVP panels"),
    ("atlas", _figure_combined_cell_lines_atlas__main,
     "Pan-cohort of DIA reanalyses (Fig S13) -- needs numpy<2"),
    ("phospho", _figure_phospho__main,
     "Phosphoproteomics supplementary figure"),
    ("venn", _venn_protein_accessions__main,
     "Protein-accession overlap (supplementary)"),
    ("performance_trace", _figure_performance_trace__main,
     "Per-step runtime + resources (runtime_per_step.svg, resources_per_step.svg)"),
    ("mdc_cluster_runtime", _figure_mdc_cluster_runtime__main,
     "MDC cluster runtime"),
]
...
def run_stage(name: str, target) -> tuple:
    """Run one stage. `target` is either an in-process callable returning an int
    return code (the inlined figure mains and recompute steps) or a module name
    string (executed as `python -m <module>` -- only the plexDIA scripts and the
    paper-numbers collector use this path)."""
    t0 = time.time()
    if callable(target):
        print(f"\n=== [{name}] (in-process) ===", flush=True)
        try:
            rc = int(target() or 0)
        except SystemExit as exc:  # figure mains may call sys.exit()
