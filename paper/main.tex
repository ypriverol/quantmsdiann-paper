...
Precursor counts (identified in at least one run) rose from 1.8.1 to 2.5.1-enterprise on every module, by 15\% on Module~7 (116{,}771~$\to$~133{,}880), 14\% on Module~9 (41{,}756~$\to$~47{,}807), 23\% on Module~5 (97{,}149~$\to$~119{,}726), and 20\% on Module~10 (88{,}179~$\to$~105{,}639).
Protein-group depth rose in step. Averaged per run, protein groups at 1\% FDR increased by 10--14\% from 1.8.1 to 2.5.1-enterprise (Module~7 10{,}091~$\to$~11{,}326; Module~9 7{,}157~$\to$~8{,}130; Module~5 10{,}199~$\to$~11{,}328; Module~10 8{,}788~$\to$~9{,}634). The gain is larger for proteins quantified in \emph{every} run (complete profiles), 12--20\% (Module~7 8{,}799~$\to$~10{,}403, $+$18\%; Module~9 6{,}124~$\to$~7{,}349, $+$20\%; complete-profile panel in Supplementary Note~5).
Reproducibility-filtered precursor counts ($\geq$3 replicates) show the same gain on all four modules.
...
On the two modules for which the snapshot contained predicted-library \texttt{DIA-NN} submissions, Orbitrap Astral (Module~7, $n=15$) and timsTOF diaPASEF (Module~5, $n=8$), \quantmsdiann{} (1.8.1 and 2.5.1-enterprise) falls within the community median-$|\epsilon|$ distribution, at or below its median (Fig.~\ref{fig:proteobench}c); the single-cell and ZenoTOF modules carried no predicted-library community submission. Per-version, per-module protein-group and precursor breakdowns are provided in Supplementary Note~5 (Fig.~S3).
