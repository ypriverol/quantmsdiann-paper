...
\begin{abstract}
Public proteomics archives now hold thousands of data-independent acquisition (DIA) datasets, but reusing them is difficult: each was processed with a different software configuration, and most lack standardised metadata. We present \quantmsdiann{}, an open-source Nextflow/nf-core workflow that runs \texttt{DIA-NN} in parallel across cloud and high-performance computing (HPC) infrastructure, guided by the experimental design declared in SDRF format. The workflow ships container-pinned profiles for each supported \texttt{DIA-NN} version through Docker and Singularity, needs no manual installation or dependency resolution, and reads all major vendor formats and the HUPO-PSI standard mzML natively. It exports harmonised quantification tables as MSstats input, the Quantitative Proteomics eXchange format (\qpx{}), and a \texttt{pmultiqc} quality-control report. The parallel design reanalyses a 2{,}300-run single-cell dataset in 2.2~hours on 300 HPC nodes. Across four ProteoBench DIA instrument modalities, distributed \quantmsdiann{} reproduces single-machine community submissions independently of \texttt{DIA-NN} release. Reanalysing public DIA deposits with a current \texttt{DIA-NN} build matches or exceeds the originally deposited counts, identifying 17\% to 59\% more protein groups per cohort, and more precursors wherever a \texttt{DIA-NN}-based original count is available. \quantmsdiann{} is a step towards scalable, reproducible reanalysis of the growing DIA data archive and a foundation for large-scale DIA meta-analysis and atlas building. It is available at \url{https://github.com/bigbio/quantmsdiann}.
\end{abstract}
...
The same pattern extends beyond bulk cell lines to another acquisition mode: a spatial Deep Visual Proteomics cohort of MYCN-amplified neuroblastoma yielded more identifications than its original \texttt{DIA-NN}~1.8.1 analysis (PXD064049; precursors 21{,}535 versus 17{,}958, $+20$\%; protein groups 3{,}301 versus 2{,}947, $+12$\%, both counted from the per-precursor reports under the same 1\% \texttt{Lib} q-value rather than the deposited count matrices, which are written at different run-specific q-values per \texttt{DIA-NN} version). Both sides count the identical 12 quantified runs. Notably, the deposited \texttt{DIA-NN}~1.8.1 analysis additionally loaded runs beyond these 12, which would tend to raise its globally identifiable protein count; even with that advantage, the 2.5.1 reanalysis identified more. Residual protein-group inference inflation is also higher in the deposited 1.8.1 analysis (3.2\% multi-accession groups) than in the reanalysis (1.3\%), so the protein-group gain is a conservative estimate. Per-cohort completeness, peptide-per-protein distributions and accession overlap for the reanalysed cohorts are provided in Supplementary Note~6 (Figs.~S4--S6).
%% [end results]

\section{Discussion}
\label{sec:discussion}
%% [begin sections/discussion.tex]
Public proteomics archives such as PRIDE and iProX are increasingly reused for new analyses, rather than accessed only for the studies that produced their datasets. Reuse of repository data can increase statistical power, connect independent cohorts, and test biological signals across laboratories~\cite{perezriverol2022reuse,hewapathirana2026datareuse}; workflow-based reanalysis has already shown how public deposits can be converted into reusable quantitative resources~\cite{dai2024quantms}, and resources such as ProteomicsDB show the value of aggregating protein-expression evidence across studies~\cite{lautenbacher2022proteomicsdb}. The need is particularly acute for DIA, a rapidly expanding class of public deposit~\cite{perezriverol2025pride} and a domain in which \texttt{DIA-NN} has become a central analysis engine~\cite{demichev2020}. Our results show that this archive can be re-mined without changing the analytical meaning of a \texttt{DIA-NN} run: across ProteoBench~\cite{devreese2025proteobench}, distributed \quantmsdiann{} reproduces the corresponding single-machine result (Fig.~\ref{fig:validation}c), so scaling across a cluster is an infrastructure choice rather than a change in analysis. When applied to public deposits processed with older or non-\texttt{DIA-NN} engines, the same workflow identifies 17\% to 59\% more protein groups, with the largest gains in older deposits. Successive \texttt{DIA-NN} releases quantify more proteins from the same spectra, so reprocessing legacy data with an improved engine increases the number of quantified proteins, an effect already reported when public datasets were reanalysed with quantms~\cite{dai2024quantms}. A substantial fraction of the proteome in legacy DIA raw data therefore remains recoverable with current, reproducible analysis.

Identification depth alone, however, does not make archived data reusable. Biological interpretation requires knowing what each sample is, how it was acquired, and how those attributes map onto the quantification matrix; the lack of structured sample metadata remains a major barrier to proteomics reuse~\cite{perezriverol2020metadata,perezriverol2022reuse,deutsch2026proteomexchange}. SDRF addresses this problem by making experimental design machine-readable~\cite{dai2021sdrf}. In \quantmsdiann{}, the SDRF drives the analysis rather than annotating it afterwards: every per-run \texttt{DIA-NN} setting is derived from the SDRF, and the same file annotates the resulting \qpx{} archive. This is what allowed the five-cohort cell-line reanalysis to be regrouped by Cellosaurus, NCIt, or Disease Ontology terms directly from the published output, with no manual harmonisation, so separately deposited studies can be queried together.

As DIA experiments grow in sample number, cohort size, and acquisition diversity~\cite{White_Paper,slavov2025Review,derks2023}, the bottleneck increasingly shifts from data generation to reproducible analysis. Archive-scale reuse requires thousands of runs to be processed quickly, with quantifications and metadata emitted in standardised, reproducible form. \quantmsdiann{} brings together SDRF-driven configuration, container-pinned \texttt{DIA-NN} profiles, and Nextflow parallelism built on established community infrastructure~\cite{perezriverol2020scalable,biocontainers2017,ewels2020nfcore}. In practice, this design reanalysed a 2{,}300-run single-cell cohort in 2.2~hours on 300 nodes, and assembled a bulk cell-line proteome of 12{,}914 protein accessions across 29 tissue categories from five independently deposited DIA datasets, each reprocessed in hours (the 5{,}798-run ProCan panel in 6.2~h); the same workflow also scales down to a small workstation analysis. Because parameters and provenance travel with the SDRF and \qpx{} archive, the output can be reproduced, inspected, or reprocessed with a different \texttt{DIA-NN} version without reconstructing per-study scripts.

By wrapping \texttt{DIA-NN} unmodified and parametrising each run from declared metadata, \quantmsdiann{} inherits the engine's full analytical range rather than being tied to a single acquisition mode or biological application. We demonstrate this across label-free single-cell proteomics, plexDIA, spatial Deep Visual Proteomics, and phosphoproteomics~\cite{brunner2022,derks2023,mund2022,makhmut2023}. Other \texttt{DIA-NN}-based applications, including DIA immunopeptidomics~\cite{scheid2025mhcquant2}, follow the same SDRF-driven path. To our knowledge, this is also the first pipeline-level treatment of cross-version \texttt{DIA-NN} reproducibility, a question that will grow as the engine continues to evolve: the same raw data and declared parameters should not lead to different biological conclusions simply because the analysis is rerun later. By design, \quantmsdiann{} stops at harmonised quantification and hands batch correction, cell-type assignment, and differential analysis to downstream ecosystems such as scp and scverse-compatible tools~\cite{vanderaa2023,virshup2023scverse}, complementing rather than replacing specialised biological analysis workflows.

\quantmsdiann{} provides infrastructure for DIA meta-analysis and archive-scale proteomics reuse. It complements earlier community efforts for DIA and quantitative-proteomics workflows~\cite{bichmann2021diaproteomics,dai2024quantms} by combining current \texttt{DIA-NN} releases, SDRF-first configuration, scalable execution, and \qpx{} export in a dedicated DIA workflow. Applied across the growing public archive, it can integrate independently deposited cohorts into reusable resources and move DIA data towards protein-expression knowledge bases~\cite{lautenbacher2022proteomicsdb} built directly from the community's accumulated measurements~\cite{perezriverol2025pride}. \quantmsdiann{} is open-source under the MIT license and developed within the nf-core and bigbio proteomics ecosystem, allowing it to track new \texttt{DIA-NN} releases, acquisition modes, and metadata requirements as they appear.
