...
To test whether \quantmsdiann{} preserves \texttt{DIA-NN}'s Desktop analytical performance under SDRF-driven orchestration, we processed every public ProteoBench DIA module through the workflow and compared the resulting identification counts against the ProteoBench public-submission snapshot (29 May 2026)~\cite{devreese2025proteobench}. Although \quantmsdiann{} supports both library-free analysis and a user-supplied spectral library, all benchmarks here were run library-free (predicting the library in-silico from the FASTA, with no externally supplied experimental library). For a like-for-like comparison we therefore restricted the community set to ProteoBench \texttt{DIA-NN} submissions that used the same predicted-from-FASTA library (\texttt{predictors\_library}~$=$~DIANN). The benchmark panel covers four instrument families and acquisition modes: Orbitrap Astral 2~Th DIA (\emph{Module~7}), single-cell DIA on Orbitrap Astral (\emph{Module~9}, PXD049412), timsTOF diaPASEF (\emph{Module~5}, PXD062685), and SCIEX ZenoTOF~7600 (\emph{Module~10}, PXD070049). For each module we focus on the oldest supported \texttt{DIA-NN} release (1.8.1) and the current enterprise build (2.5.1-enterprise), run in a single invocation and parametrised from the same SDRF (Fig.~\ref{fig:proteobench}). The intermediate academic 2.5.1 build was run alongside and, with the full per-version breakdown, is reported in Supplementary Note~5 (Fig.~S3). Identification counts are taken from the per-precursor \texttt{DIA-NN} report files. Each release is run at \texttt{DIA-NN}'s recommended run-specific precursor q-value (\texttt{--qvalue}: 1\% for 1.8.1, 5\% for the 2.5.1 builds), with a uniform 1\% global precursor q-value across configurations; we count unique precursors identified in at least one run and summarise protein groups (each at a 1\% run-specific protein-group q-value) as the per-run average and the ``complete-profile'' count quantified in \emph{every} run. The single-replicate precursor totals and per-run protein metrics apply identical thresholds to every version and are directly comparable; only the reproducibility-filtered ($\geq$3-replicate) precursor counts depend on the per-version run-specific cut-off.

Precursor counts (identified in at least one run) rose from 1.8.1 to 2.5.1-enterprise on every module, by 15\% on Module~7 (116{,}771~$\to$~133{,}880), 14\% on Module~9 (41{,}756~$\to$~47{,}807), 23\% on Module~5 (97{,}149~$\to$~119{,}726), and 20\% on Module~10 (88{,}179~$\to$~105{,}639). Averaged per run, protein groups at 1\% FDR increased by 10--14\% from 1.8.1 to 2.5.1-enterprise (Module~7 10{,}091~$\to$~11{,}326; Module~9 7{,}157~$\to$~8{,}130; Module~5 10{,}199~$\to$~11{,}328; Module~10 8{,}788~$\to$~9{,}634). The gain is larger for proteins quantified in \emph{every} run (complete profiles), 12--20\% (Module~7 8{,}799~$\to$~10{,}403, $+$18\%; Module~9 6{,}124~$\to$~7{,}349, $+$20\%; complete-profile panel in Supplementary Note~5). Reproducibility-filtered precursor counts ($\geq$3 replicates) show the same gain on all four modules.
...
The purpose of this benchmark is not to contrast \texttt{DIA-NN} versions but to show that distributing \texttt{DIA-NN} across a cluster reproduces what the same engine yields on a single desktop machine: independent of the release used, \quantmsdiann{} quantifies the ProteoBench modules indistinguishably from the single-machine community submissions.
Quantification accuracy supports this. On every module the measured per-species log$_2$ fold-changes recover the expected HYE ratios (human at unity, yeast and \emph{E.\ coli} near their design ratios, with the mild attenuation at the extreme \emph{E.\ coli} ratio that is characteristic of DIA quantification; Fig.~\ref{fig:proteobench}b).
On the two modules for which the snapshot contained predicted-library \texttt{DIA-NN} submissions, Orbitrap Astral (Module~7, $n=15$) and timsTOF diaPASEF (Module~5, $n=8$), \quantmsdiann{} falls within the community median quantification-error~$|\epsilon|$ distribution, at or below its median (Fig.~\ref{fig:proteobench}c); its distributed runs are therefore statistically interchangeable with the desktop community runs.
This equivalence is independent of \texttt{DIA-NN} release: the version-to-version shift in median~$|\epsilon|$ ($\sim$0.01 log$_2$, $\approx$0.5--1\% in linear fold-change) is far smaller than the community spread, so the 1.8.1 and 2.5.1-enterprise builds overlie one another at every species (Fig.~\ref{fig:proteobench}b) and both sit inside the community cloud. The single-cell and ZenoTOF modules carried no predicted-library community submission, and the per-version, per-module breakdowns are provided in Supplementary Note~5 (Fig.~S3).
...
The pipeline is implemented in Nextflow DSL2 ($\geq$25.10.4) and packages each step as a versioned Docker, Singularity, or Apptainer container image. \texttt{DIA-NN}~1.8.1 is distributed publicly via BioContainers; because the \texttt{DIA-NN} license restricts redistribution of later releases, containers for \texttt{DIA-NN}~1.9 and above are built locally from the recipes in \url{https://github.com/bigbio/quantms-containers} (see Experimental Procedures). Pipeline version v2.1.0 (release codename ``Sao Pablo'', 2026-05-08) was used for the analyses reported in this paper. Analysis scripts that generated the figures in this manuscript are available at \url{https://github.com/bigbio/quantmsdiann-paper} (this repository).
