...
Reanalysis of public proteomics data is becoming part of the scientific process of multiple labs; not only to complement local data; but also produce more robust biological insigths, models etc [https://www.biorxiv.org/content/10.64898/2026.04.16.718670v1, PMID: 36529941]. DIA is becoming the Reanalysing archived DIA data with a current engine delivers two gains at once. Where a deposit was already processed with \texttt{DIA-NN}, distributed \quantmsdiann{} reproduces the single-machine result (Fig.~\ref{fig:validation}c), so moving from a desktop run to a cluster is a deployment choice rather than an analytical trade-off. Where a deposit was processed with an older or non-\texttt{DIA-NN} engine, reprocessing recovers substantially more, up to 59\% more protein groups, and the gain is largest exactly where deposits are most numerous: the large back-catalogue of ageing DIA data. The same workflow therefore both preserves results the community already trusts and extracts new ones from data that has already been collected and published.

These gains matter only if they are easy to obtain. \quantmsdiann{} packages the entire analysis behind a single SDRF file and container-pinned \texttt{DIA-NN} profiles, so a reanalysis needs no manual installation, parameter harmonisation, or per-study scripting, and the same invocation runs unchanged from a six-run benchmark to thousands of runs across cloud and HPC. This makes it an accessible, reproducible platform for analysing both public deposits and a laboratory's own unpublished cohorts en masse. Because the per-file work is parallelised, throughput is bounded only by the compute dedicated to it: a 2{,}300-run single-cell cohort completes in 2.2~hours on 300 nodes, and the same pipeline scales down to a handful of runs on a workstation.
...
Looking ahead, a workflow that turns any SDRF-annotated deposit into harmonised, queryable quantifications is well suited to powering DIA reanalysis at archive scale. Applied across the growing body of public data, it can integrate independently deposited cohorts into a single reusable resource, a step towards a proteomics knowledge base that aggregates the community's accumulated measurements rather than leaving them siloed in individual deposits.
