...
Reanalysis of public proteomics data has become part of how the field produces knowledge, not only to complement locally generated data but to build more robust, cross-cohort biological models from measurements that already exist~\cite{perezriverol2022reuse,hewapathirana2026datareuse}. This shift is most consequential for DIA, now among the fastest-growing classes of public deposit~\cite{perezriverol2025pride}: reusable raw data is accumulating faster than it is being re-mined. Our results quantify what that back-catalogue holds. Reanalysing archived DIA data with a current engine delivers two gains at once. Where a deposit was already processed with \texttt{DIA-NN}, distributed \quantmsdiann{} reproduces the single-machine result (Fig.~\ref{fig:validation}c), so moving from a desktop run to a cluster is a deployment choice rather than an analytical trade-off. Where a deposit was processed with an older or non-\texttt{DIA-NN} engine, reprocessing recovers substantially more, up to 59\% more protein groups, and the gain is largest exactly where deposits are most numerous: the large back-catalogue of ageing DIA data.

Recovering more identifications is necessary but not sufficient. Turning pooled public data into biology requires knowing what each sample is, and the scarcity of structured sample metadata remains a central obstacle to reuse~\cite{perezriverol2022reuse}. Community standards such as SDRF~\cite{dai2021sdrf} make experimental design machine-readable, and \quantmsdiann{} treats the SDRF as the single source of truth: every per-run \texttt{DIA-NN} setting is derived from it, so one file both drives the analysis and annotates its output. This is what let the five-cohort reanalysis be regrouped by Cellosaurus, NCIt, or Disease Ontology terms directly from the published \qpx{} archive, with no manual harmonisation, turning independently deposited studies into a single queryable resource.

As sample numbers and cohort sizes grow, the analytical bottleneck shifts from the instrument to the reanalysis: extracting value at archive scale requires reprocessing thousands of runs quickly and emitting both the quantifications and their metadata in a standardised, queryable form. \quantmsdiann{} is built for this. A single SDRF and container-pinned \texttt{DIA-NN} profiles remove manual installation, parameter harmonisation, and per-study scripting; the parallel design bounds throughput only by the compute dedicated to it, reanalysing a 2{,}300-run single-cell cohort in 2.2~hours on 300 nodes and scaling down unchanged to a handful of runs on a workstation; and because every parameter travels in the SDRF and \qpx{} archive, each result carries the provenance needed to reproduce or audit it. Dedicating an independently released pipeline to \texttt{DIA-NN}, rather than sharing one workflow across acquisition types, is what makes this combination of speed, simplicity, and provenance achievable.

Because \quantmsdiann{} wraps \texttt{DIA-NN} unmodified and parametrises each run from its declared metadata, it inherits the engine's full analytical range. We demonstrate this breadth here across label-free single-cell, multiplexed plexDIA, spatial Deep Visual Proteomics, and phosphoproteomic cohorts, each analysed from its own SDRF without any workflow change; other \texttt{DIA-NN} applications, such as DIA immunopeptidomics~\cite{scheid2025mhcquant2}, follow the same path. To our knowledge this is also the first pipeline-level treatment of cross-version \texttt{DIA-NN} reproducibility, a question that grows in importance as the engine evolves: a dataset analysed with \diannversion{1.8} today and reprocessed with \diannversion{2.x} in two years should not yield different biological conclusions for the same data and parameters. By design, \quantmsdiann{} ends at harmonised quantification and hands batch correction, cell-type assignment, and differential analysis to downstream packages such as scp~\cite{vanderaa2023} that read its \qpx{} or MSstats output directly, complementing rather than replacing existing workflows.

Looking ahead, a workflow that turns any SDRF-annotated deposit into harmonised, queryable quantifications is well suited to powering DIA reanalysis at archive scale. Applied across the growing body of public data, it can integrate independently deposited cohorts into a single reusable resource, a step towards protein-expression knowledge bases~\cite{lautenbacher2022proteomicsdb} built directly on the community's accumulated measurements~\cite{perezriverol2025pride} rather than leaving them siloed in individual deposits. \quantmsdiann{} is open-source under the MIT license and openly developed within the nf-core and bigbio proteomics ecosystem, so it can track new \texttt{DIA-NN} releases and acquisition modes as they appear.
