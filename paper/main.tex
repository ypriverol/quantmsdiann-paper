...
To assess \quantmsdiann{} on low-input data, where library-free sensitivity and match-between-runs matter most, we reanalysed a label-free single-cell HeLa cohort, Orbitrap Astral single-cell (PXD046357), through the workflow under \texttt{DIA-NN}~1.8.1 and the current 2.5.1-enterprise build, both parametrised from the same SDRF (Fig.~\ref{fig:single-cell}).
The newer 2.5.1-enterprise version increased total identifications: on HeLa Astral, total precursors rose from 22{,}531 to 25{,}145 ($+12$\%) and protein groups from 4{,}134 to 4{,}735 ($+15$\%), with the gain holding per cell (Fig.~\ref{fig:single-cell}a).
...
%% Fig. 3 — Reanalysis recovers more than the original deposits (main message)
\begin{figure}[htbp]
  \centering
...
%% Fig. 4 — Single-cell DIA reanalysis (DIA-NN 1.8.1 vs 2.5.1-enterprise)
\begin{figure}[htbp]
  \centering
  ...
  \caption[Single-cell DIA reanalysis]{%
    ...
    raw data.} The label-free Orbitrap Astral single-cell HeLa cohort
    (PXD046357) is shown.
    ...
    build lifting both the totals and per-cell depth; the per-build
    percentage change is annotated.
    (\textbf{b})~Data-completeness curve: protein groups quantified in at least
    $N$ cells, the match-between-runs signature, where the right endpoint is the
    complete-profile count.
    (\textbf{c})~Distribution of per-protein coefficient of variation across
    cells, summarising quantitative precision.
  }
