...
Identification depth, by contrast, increases with each \texttt{DIA-NN} version on every module. Per-run protein groups rise consistently from 1.8.1 through 2.5.1 to the enterprise build across all four DIA modalities (Fig.~\ref{fig:validation}d), a gain of 10--12\%: for example, from 7{,}305 to 8{,}190 protein groups per run on the single-cell Astral module (Module~9) and from 10{,}192 to 11{,}432 on Orbitrap Astral (Module~7). Precursors and the number of proteins quantified in every run increase in parallel (per-version precursor counts and the complete-profile breakdown in Supplementary Note~5, Fig.~S3). This tracks the broader ProteoBench community trend~\cite{devreese2025proteobench} and motivates reanalysis of public deposits with the current build.
