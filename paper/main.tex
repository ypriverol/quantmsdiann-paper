...
\label{sec:proteobench}
 ...
 To test whether \quantmsdiann{} preserves \texttt{DIA-NN}'s desktop performance under SDRF-driven orchestration, we processed the four public ProteoBench DIA modules (Orbitrap Astral, \emph{Module~7}; single-cell Astral, \emph{Module~9}, PXD049412; timsTOF diaPASEF, \emph{Module~5}, PXD062685; SCIEX ZenoTOF~7600, \emph{Module~10}, PXD070049) and compared the per-precursor identification counts against the ProteoBench public-submission snapshot (29 May 2026)~\cite{devreese2025proteobench}. All runs were library-free (the library predicted in-silico from the FASTA; \quantmsdiann{} also supports user-supplied libraries), so for a like-for-like comparison we kept only predicted-from-FASTA \texttt{DIA-NN} community submissions. We report the oldest supported release (1.8.1) and the current 2.5.1-enterprise build, at \texttt{DIA-NN}'s recommended per-version q-values under a uniform 1\% global precursor cut-off (Fig.~\ref{fig:proteobench}); the academic 2.5.1 build and the full per-version, per-module breakdown are in Supplementary Note~5 (Fig.~S3).

The aim is not to contrast \texttt{DIA-NN} versions but to show that distributing \texttt{DIA-NN} across a cluster reproduces what the same engine yields on a single desktop machine. On the two modules with predicted-library community submissions (Module~7, $n=15$; Module~5, $n=8$; Modules~9 and~10 had none), \quantmsdiann{} recovers the expected HYE per-species log$_2$ fold-changes (Fig.~\ref{fig:proteobench}b) and falls within the community median quantification-error~$|\epsilon|$ distribution, at or below its median (Fig.~\ref{fig:proteobench}c). This holds independent of release: the 1.8.1 and 2.5.1-enterprise builds overlie one another and both sit inside the community cloud, the version-to-version shift in median~$|\epsilon|$ ($\approx$0.5--1\% in linear fold-change) being far smaller than the community spread.

Newer releases gain depth without changing this accuracy: from 1.8.1 to 2.5.1-enterprise, precursors identified in at least one run rose 15--23\% and per-run protein groups 10--14\% across the four modules (12--20\% for proteins quantified in every run), the same monotonic trend the broader ProteoBench community reports across \texttt{DIA-NN} releases~\cite{devreese2025proteobench}.
