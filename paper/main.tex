...
Reanalysis of public proteomics data has become part of how the proteomics field produces knowledge, not only to complement locally generated data but to build more robust, cross-cohort biological models from measurements that already exist~\cite{perezriverol2022reuse,hewapathirana2026datareuse}. This is especially true for DIA, now among the fastest-growing classes of public deposit~\cite{perezriverol2025pride}, where reusable raw data is accumulating faster than it is being reused. What that back-catalogue still holds depends on how each deposit was originally analysed. Where a deposit was already processed with \texttt{DIA-NN}, distributed \quantmsdiann{} reproduces the single-machine result (Fig.~\ref{fig:validation}c), so scaling across a cluster is a deployment choice rather than an analytical trade-off. Where the original analysis used an older or non-\texttt{DIA-NN} engine, reprocessing recovers substantially more, up to 59\% more protein groups, and the gain is largest on the oldest deposits, the ageing back-catalogue that dominates the public archive.
...
As sample numbers and cohort sizes grow, the analytical bottleneck shifts from the instrument to the reanalysis: extracting value at archive scale requires reprocessing thousands of runs quickly and emitting both the quantifications and their metadata in a standardised, queryable form. \quantmsdiann{} was designed for exactly this workload. A single SDRF and container-pinned \texttt{DIA-NN} profiles remove manual installation, parameter harmonisation, and per-study scripting; the parallel design bounds throughput only by the compute dedicated to it, reanalysing a 2{,}300-run single-cell cohort in 2.2~hours on 300 nodes and scaling down unchanged to a handful of runs on a workstation; and because every parameter travels in the SDRF and \qpx{} archive, each result carries the provenance needed to reproduce or audit it. Reaching this level of speed and simplicity is why we dedicated an independent pipeline to \texttt{DIA-NN} rather than extending a workflow shared across acquisition types.
